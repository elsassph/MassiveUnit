...
massive.munit.client.HTTPClient.get_hasQueue = function() {
	return massive.munit.client.HTTPClient.responsePending || massive.munit.client.HTTPClient.queue.length > 0;
};
...
massive.munit.client.HTTPClient.prototype = {
	...
	,reportFinalStatistics: function(testCount,passCount,failCount,errorCount,ignoreCount,time) {
		...
		console.log("POSTING RESULTS OF " + this.client.id + " - queued? " + Std.string(this.queueRequest));
		if(typeof(result) == "string") {
			...
			if(size > 500) {
				...
				while(pos < size) {
					var part;
					if(pos + 500 >= size) part = "eof"; else part = "partial";
					console.log("PART " + part + " " + pos);
					this.sendResult(HxOverrides.substr(data,0,500),part);
					pos += 500;
				}
				return result;
			}
	...
	,sendResult: function(result,part) {
		...
		if(part != null) this.request.setHeader("munit-multipart",part);
	...
	,onData: function(data) {
		if(this.queueRequest) {
			...
			massive.munit.client.HTTPClient.dispatchNextRequest();
			if(massive.munit.client.HTTPClient.responsePending) return;
		}
		console.log("HTTClient " + this.client.id + " is complete");
...
massive.munit.client.HTTPClient.PARTIAL_SIZE = 500;
